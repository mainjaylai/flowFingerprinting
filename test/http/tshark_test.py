...
# sudo tshark -i enp0s8 -f "tcp port 8080" -w tshark_output.pcap

# 监控 tshark 进程持续时间
capture_duration = 60

# 启动 tshark 进程捕获 HTTP 流量
tshark_command = ["tshark", "-i", "enp0s8", "-f", "tcp port 8080", "-a", f"duration:{capture_duration}", "-w", "tshark_output.pcap"]
